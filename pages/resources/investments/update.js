...
export default function Add(props) {
  ...
  const getmembers = async () => {
    ...
    if (res1?.success) {
      ...
      getGoalData(dummyNow, [{ ...dummyNow }, ...newary]);
    } else {
      setLoading(false);
      setMembers([]);
    }
  ...
  const getGoalData = async (resources_time, starttimeline) => {
    setLoading(true);
  ...
  const getinvestment = async (starttimeline) => {
    setLoading(true);
    let response = await fetch(`/api/resources/get-investment-byid`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ assetId: props?.investmentId }),
    });
    let res1 = await response.json();
    if (res1?.success) {
      let data = res1?.data;
      ...
      let starttime = {
        ...
        value: parseInt(data?.resources_access_time?.value),
        member: {
      ...
      setValues({
        ...values,
        type: data?.type,
        curr_valuation: data?.curr_valuation,
        name: data?.name,
        goals: [...dummygoals],
        goalselected: data?.goal_state,
        goalselected: data?.surplusgoal_state,
        surplus_goals: [...dummysurgoals],
        owner: {
...
export const getServerSideProps = async (ctx) => {
  ...
  let investmentId = ctx?.query?.investmentId;
  return { props: { investmentId: investmentId, user: user } };
};
