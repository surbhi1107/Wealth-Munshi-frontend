...
export default function UpdateDept(props) {
  const router = useRouter();
  let liabilityId = props?.liabilityId;
  let user = props.user;
  ...
  const {
    ...
    onSubmit: async (values) => {
      try {
        ...
        const res = await fetch(`/api/debt/update`, {
          method: "POST",
          headers: {
  ...
  const getDebtdata = async () => {
    let response = await fetch(`/api/debt/get-debt-byid`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ liabilityId: liabilityId }),
    });
    let res1 = await response.json();
    if (res1?.success) {
      let data = res1?.data;
      setLoading(false);
      let damipayment = data?.payments.map((v) => {
        let findtimeline = timelineLists?.find(
        ...
        return {
          ...
          isin_cashflow: v?.isin_cashflow,
          timeline: {
            label: findtimeline?.label,
            value: findtimeline?.value,
          },
          next_payment_start: {
            type: v?.next_payment_start?.type,
            value: v?.next_payment_start?.value,
            ...
            ...(v?.next_payment_start?.type !== "year"
              ? {
                  member: {
                    ...
                    life_expectancy:
                      v?.next_payment_start?.member?.life_expectancy,
                  },
                }
              : {}),
          },
      ...
      setValues({
        ...
        owner: {
          label: data?.owner?.fname,
          value: data?.owner?._id,
        },
        payments: [...damipayment],
      });
    } else {
      errorToast(res1?.error);
      setLoading(false);
...
export const getServerSideProps = async (ctx) => {
  ...
  if (!ctx?.query?.liabilityId) {
    return {
      redirect: {
        destination: "/resources/debts",
        permanent: false,
      },
    };
  }
  let liabilityId = ctx?.query?.liabilityId;
  return { props: { liabilityId: liabilityId, user: user } };
};
