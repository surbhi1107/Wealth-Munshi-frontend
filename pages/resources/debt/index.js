...
export default function Dept(props) {
  ...
  const deleteDebt = async (_id) => {
    try {
      setLoading(true);
      let response = await fetch(`/api/debt/delete`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ liabilityId: _id }),
      });
      let res1 = await response.json();
      if (res1?.success) {
        setLoading(false);
                    ...
                    >
                      ...
                      >
                        <button
                          ...
                          onClick={() => {
                            router.push({
                              pathname: "/resources/debt/update/",
                              query: { liabilityId: v._id },
                            });
                          }}
                        >
                          <svg
        ...
        >
          <div className="w-full grid grid-cols-2 md:grid-cols-3 gap-5">
            {deptTypes.map((v, i) => (
              <Link
                href={{
                  pathname: `/resources/debt/add`,
                  query: { debt_type: JSON.stringify(v) },
                }}
                key={i}
                as={`/resources/debt/add`}
