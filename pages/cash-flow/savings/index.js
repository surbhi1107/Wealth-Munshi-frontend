...
export default function GetSavings(props) {
  ...
  const getData = async () => {
    setLoading(true);
    let response = await fetch(`/api/savings/get-all-savings`, {
      method: "POST",
      headers: {
    ...
    if (res1?.data) {
      setLoading(false);
      setDebtData(res1?.data);
    } else {
      setLoading(false);
    }
  };

  const deleteDebt = async (_id) => {
    try {
      setLoading(true);
      let response = await fetch(`/api/savings/delete`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ savingId: _id }),
      });
      let res1 = await response.json();
      if (res1?.success) {
        setLoading(false);
      ...
      <div className={`w-full space-y-6`}>
        ...
        <div className="w-full px-[30px] py-[30px] bg-white rounded-md space-y-6">
          ...
          <div className="w-full md:!mt-3">
            <ul className="text-[#A1A1AA] space-y-3 list-outside text-justify">
              <li>
                Enter here any savings to which you are already committed.
              </li>
              <li>
                Committed savings associated with an investment will already be
                carried through from the Financial Resources section.
              </li>
            </ul>
          </div>
        </div>
        <div className="w-full px-[30px] py-[30px] bg-white rounded-md space-y-6">
          <div className="flex space-x-3">
            <Link
              href={{
                pathname: `/cash-flow/savings/add`,
                query: { saving_type: "deposite" },
              }}
              as={`/cash-flow/savings/add`}
                  ...
                  deptData?.map((v, index) => (
                    ...
                    >
                      ...
                      <td className={`py-1 px-3 whitespace-nowrap capitalize`}>
                        {v?.type === "deposite"
                          ? `Deposite into ${v?.asset?.type?.replace(
                              /_/g,
                              " "
                            )} (${
                              v?.asset?.owner?.fname
                            }'s ${v?.asset?.type?.replace(/_/g, " ")})`
                          : `Regular Deposite into ${v?.asset?.type?.replace(
                              /_/g,
                              " "
                            )} (${
                              v?.asset?.owner?.fname
                      ...
                      <td className={`py-1 px-3 whitespace-nowrap`}>
                        {`Rs. ${v?.amount} ${v?.saving_start?.desc}`}
                      </td>
                      <td
                      ...
                      >
                        <button
                          ...
                          onClick={() => {
                            router.push({
                              pathname: "/cash-flow/savings/update/",
                              query: { savingId: v._id },
                            });
                          }}
                        >
                          <svg
