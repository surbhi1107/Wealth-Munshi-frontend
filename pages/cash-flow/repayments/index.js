...
export default function GetRepayments(props) {
  ...
  const getData = async () => {
    setLoading(true);
    let response = await fetch(`/api/payments/get-all-payments`, {
      method: "POST",
      headers: {
  ...
  const deleteDebt = async (_id) => {
    try {
      setLoading(true);
      let response = await fetch(`/api/payments/delete`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ paymentId: _id }),
      });
      let res1 = await response.json();
      if (res1?.success) {
        setLoading(false);
      ...
      <div className={`w-full space-y-6`}>
        ...
        <div className="w-full px-[30px] py-[30px] bg-white rounded-md space-y-6">
          ...
          <div className="w-full md:!mt-3">
            <ul className="text-[#A1A1AA] space-y-3 list-outside text-justify">
              <li>
                Enter here any debt repayments to which you are committed.
              </li>
              <li>
                Debt repayments associated with assets will already be carried
                through from the Financial Resources section.
              </li>
            </ul>
          </div>
        </div>
        <div className="w-full px-[30px] py-[30px] bg-white rounded-md space-y-6">
          <div className="flex space-x-3">
            <Link
              href={{
                pathname: `/cash-flow/repayments/add`,
                query: { payment_type: "oneoff" },
              }}
              as={`/cash-flow/repayments/add`}
                  ...
                  deptData?.map((v, index) => (
                    ...
                    >
                      ...
                      <td className={`py-1 px-3 whitespace-nowrap capitalize`}>
                        {v?.liability?._id
                          ? `Repayments into ${
                              v?.liability?.owner?.fname
                            }'s ${v?.liability?.type?.replace(/_/g, " ")}`
                          : `Repayment`}
                      </td>
                      <td className={`py-1 px-3 whitespace-nowrap`}>
                        {`Rs. ${v?.amount} ${v?.next_payment_start?.desc}`}
                      </td>
                      <td
                      ...
                      >
                        <button
                          ...
                          onClick={() => {
                            router.push({
                              pathname: "/cash-flow/repayments/update/",
                              query: { paymentId: v._id },
                            });
                          }}
                        >
                          <svg
