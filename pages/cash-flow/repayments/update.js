...
export default function UpdateSaving(props) {
  ...
  const [debtData, setDebtsData] = useState([]);
    ...
    onSubmit: async (values) => {
      try {
        ...
        let dummystartfind = startTime.find(
          (v) => v.type === values.next_payment_start.type
        );
        let dummyendfind = endTime.find(
          (v) => v.type === values.next_payment_end.type
        );
        let data = {
          type: values.type,
          name: values.name,
          ...(values?.liability?.value === "0"
            ? {}
            : { liability_id: values.liability?.value }),
        ...
        if (res1.success) {
          router.push("/cash-flow/repayments");
          setLoading(false);
        } else {
          setLoading(false);
  ...
  const getData = async () => {
    setLoading(true);
    let response = await fetch(`/api/debt/get-all-debt`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (res1?.success) {
      ...
      let dummyasset = res1?.data?.map((v) => {
        return {
          ...
          value: v?._id,
          curr_amount: v?.curr_amount,
        };
      });
      setDebtsData(dummyasset);
      getmembers();
    } else {
      setLoading(false);
      setDebtsData([]);
  ...
  const getmembers = async () => {
    ...
    if (res1?.success) {
      ...
      getpaymentData(newary, [...newary, dummyafteryear]);
    } else {
      setLoading(false);
      setMembers([]);
    }
  ...
  const getpaymentData = async (oldstarttime, oldendstart) => {
    setLoading(true);
    let response = await fetch(`/api/payments/get-payment-byid`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    if (res1?.success) {
      ...
      setLoading(false);
      let starttime = {
        ...data.next_payment_start,
        value: parseInt(data?.next_payment_start?.value),
        member: {
      ...
      let endttime = {
        ...data?.next_payment_end,
        value: parseInt(data?.next_payment_end?.value),
        member: {
      ...
      let damitimeline = timelineLists.find((v) => v.value === data?.timeline);
      setValues({
        type: data.type,
        name: data?.name ?? "",
        amount: data?.amount ?? "",
        inflation: data?.inflation ?? "",
        isin_cashflow: data?.isin_cashflow,
        ...(data?.liability?._id?.length > 0
          ? {
              liability: {
                label: data?.liability?.fname,
                value: data?.liability?._id,
                curr_amount: data?.liability?.curr_amount,
              },
            }
          : {}),
        next_payment_start: {
        ...
        ...(data?.type === "regular"
          ? {
              timeline: {
                label: damitimeline?.label,
                value: damitimeline?.value,
              },
              next_payment_end: {
                ...endttime,
              },
          ...
          {loading ? (
            ...
            >
              <div className="md:grid md:grid-cols-2 gap-x-8 gap-y-3">
                ...
                />
                <Dropdown
                  placeholder="--Not contributing to a liability--"
                  label={"Associated liability"}
                  options={debtData}
                  value={values.liability}
                  onchange={(e) => {
                    let val = e.target.value;
                    let dummydebt = debtData?.find((v) => v.value === val);
                    setFieldValue("liability", {
                      label: dummydebt?.label,
                      value: dummydebt?.value,
                      curr_amount: dummydebt?.curr_amount,
                    });
                  }}
                  error={
                    touched.liability?.value && errors.liability?.value
                      ? true
                      : false
                  }
                  errorText={errors.liability?.value}
                  require
                ...
                <div className="md:flex md:col-span-2 items-center md:gap-x-8 gap-y-3">
                  ...
                  {values.type === "regular" ? (
                    <Dropdown
                      ...
                      mainClass="w-max"
                      error={
                        touched.income_owner?.value &&
                        errors.income_owner?.value
                          ? true
                          : false
                      }
                    ...
                    <div className="flex gap-2 items-center">
                      <span className="relative flex items-center rounded-full cursor-pointer">
                        <input
                          id={"year"}
                          checked={
                            values.next_payment_start?.type === "year"
                              ? true
                              : false
                          }
                          onChange={() => {
                            ...
                            setFieldValue("next_payment_start", findtype);
                          }}
                          name="next_payment_start"
                          type="radio"
                    ...
                    <div className="flex gap-2 items-center">
                      <span className="relative flex items-center rounded-full cursor-pointer">
                        <input
                          id={"age"}
                          checked={
                            values.next_payment_start?.type === "age"
                              ? true
                              : false
                          }
                          onChange={() => {
                    ...
                    <div className="flex gap-2 items-center">
                      <span className="relative flex items-center rounded-full cursor-pointer">
                        <input
                          id={"age_retire"}
                          checked={
                            values.next_payment_start?.type === "age_retire"
                              ? true
                              : false
                          }
                          onChange={() => {
                    ...
                    <div className="flex gap-2 items-center">
                      <span className="relative flex items-center rounded-full cursor-pointer">
                        <input
                          id={"life_exp"}
                          checked={
                            values.next_payment_start?.type === "life_exp"
                              ? true
                              : false
                          }
                          onChange={() => {
                      ...
                      <div className="flex gap-2 items-center">
                        <span className="relative flex items-center rounded-full cursor-pointer">
                          <input
                            id={"endyear"}
                            checked={
                              values.next_payment_end?.type === "year"
                                ? true
                                : false
                            }
                            onChange={() => {
                              ...
                              setFieldValue("next_payment_end", findtype);
                            }}
                            name="next_payment_end"
                            type="radio"
                            className="before:content[''] peer relative -gray h-4 w-4 cursor-pointer appearance-none rounded-full border border-[#757575] before:absolute before:top-2/4 before:left-2/4 before:block before:h-[17px] before:w-[17px] before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-[#57BA52] before:opacity-0 before:transition-opacity checked:border-[#57BA52] checked:before:bg-[#57BA52]"
                          />
                          <span className="absolute text-[#57BA52] transition-opacity opacity-0 pointer-events-none top-2/4 left-2/4 -translate-y-2/4 -translate-x-2/4 peer-checked:opacity-100">
                      ...
                      <div className="flex gap-2 items-center">
                        <span className="relative flex items-center rounded-full cursor-pointer">
                          <input
                            id={"endage"}
                            checked={
                              values.next_payment_end?.type === "age"
                                ? true
                                : false
                            }
                            onChange={() => {
                      ...
                      <div className="flex gap-2 items-center">
                        <span className="relative flex items-center rounded-full cursor-pointer">
                          <input
                            id={"endage_retire"}
                            checked={
                              values.next_payment_end?.type === "age_retire"
                                ? true
                                : false
                            }
                            onChange={() => {
                      ...
                      <div className="flex gap-2 items-center">
                        <span className="relative flex items-center rounded-full cursor-pointer">
                          <input
                            id={"endlife_exp"}
                            checked={
                              values.next_payment_end?.type === "life_exp"
                                ? true
                                : false
                            }
                            onChange={() => {
                      ...
                      <div className="flex gap-2 items-center">
                        <span className="relative flex items-center rounded-full cursor-pointer">
                          <input
                            id={"afteryears"}
                            checked={
                              values.next_payment_end?.type === "afteryear"
                                ? true
                                : false
                            }
                            onChange={() => {
...
export const getServerSideProps = async (ctx) => {
  ...
  if (!ctx?.query?.paymentId) {
    return {
      redirect: {
        destination: "/cash-flow/repayments",
        permanent: false,
      },
    };
  }
  let paymentId = ctx?.query?.paymentId;
  return { props: { paymentId: paymentId, user: user } };
};
